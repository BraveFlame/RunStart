...
//        } else if(str.equals("3"))CirclePhotoUtils.showImage(imageView,bxfPath);
...
public class CirclePhotoUtils {
    ...
    static public String myPictureOnResultOperate(int requestCode, int resultCode, Intent data, Activity context){
        ...
        if (resultCode == CirclePhotoUtils.NONE){
            return "0";
        }
        // /////////////////////////////////////////拍照///////////////////////////////////////////////
        if (requestCode == CirclePhotoUtils.PHOTOGRAPH) {
            // 设置文件保存路径这里放在跟目录下
            File picture;
            if (Environment.MEDIA_MOUNTED.equals(Environment.getExternalStorageState())) {
                picture = new File(Environment.getExternalStorageDirectory() + CirclePhotoUtils.imageName);
                if (!picture.exists()) {
                    picture = new File(Environment.getExternalStorageDirectory() + CirclePhotoUtils.imageName);
                }
            } else {
                picture = new File(context.getFilesDir() + CirclePhotoUtils.imageName);
                if (!picture.exists()) {
                    picture = new File(context.getFilesDir() + CirclePhotoUtils.imageName);
                }
            }

            BxfPath = CirclePhotoUtils.getPath(context);// 生成一个地址用于存放剪辑后的图片
            ...
            CirclePhotoUtils.startPhotoZoom(context, Uri.fromFile(picture),328,116, height, width, imageUri);
            return "bxf"+BxfPath;
        }
        if (data == null)
            return "4";
        ////////////////////////////// 读取相册缩放图片///////////////////////////////////////////////////////////
        if (requestCode == CirclePhotoUtils.PHOTOZOOM) {
            BxfPath = CirclePhotoUtils.getPath(context);// 生成一个地址用于存放剪辑后的图片
            if (TextUtils.isEmpty(BxfPath)) {
                Log.e(TAG, "随机生成的用于存放剪辑后的图片的地址失败");
                return "2";
            ...
            int width = wm.getDefaultDisplay().getWidth();
            int height = wm.getDefaultDisplay().getHeight();
            CirclePhotoUtils.startPhotoZoom(context, data.getData(),328,116, height, width, imageUri);
        }
        /////////////////////////////// 处理裁剪结果////////////////////////////////////////////////////////////////
        if (requestCode == CirclePhotoUtils.PHOTORESOULT) {
            /**
    ...
    static public void showImage(ImageView foreImageView,ImageView backImageView, String path){
    ...
    static public void options(final Activity activity) {
        new AlertDialog.Builder(activity).setTitle("选择").setCancelable(true)
            .setItems(new String[]{"拍照", "相册"}, new DialogInterface.OnClickListener() {
                @Override
                public void onClick(DialogInterface dialog, int which) {
                    if (which == 0){
                        try {
                            CirclePhotoUtils.photograph(activity);
                        }catch (Exception e){
                            e.printStackTrace();
                        }
                    }else {
                        CirclePhotoUtils.selectPictureFromAlbum(activity);
                    }
                }
            }).show();
    }
